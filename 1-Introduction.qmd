...
- `observed`: 観測度数
- `expected`: 期待度数
- `residuals`: ピアソン残差 $(f_{ij} - F_{ij}) / \sqrt{F_{ij}}$
- `stdres`: 標準化残差 $(f_{ij} - F_{ij}) / \sqrt{V_{ij}}$，ここで$V_{ij} = F_{ij}(1 - p_{i+})(1 - p_{+j})$（$p_{i+}$, $p_{+j}$は周辺比率）

各要素には`$`演算子でアクセスできる．例えば期待度数は`result$expected`で取得できる．これを`tab_expected`というオブジェクトとして保存する．

```{r}
result <- chisq.test(tab)
tab_expected <- result$expected |> as.table()
tab_expected
```

せっかくなので全部確認してみたい．`unclass()`はオブジェクトのクラスを外し，全要素をリストとして出力する（`result[names(result)]`と同じ効果）．
...
`Margins()`で期待度数の周辺分布を確認する．これは元の観測度数の周辺分布と一致する．
...
対数線形モデルでは，ある効果の自由度を使い切ると，対応する度数が観測値と期待値で完全に一致する．これは「十分統計量（sufficient statistic）の性質」として知られ，最尤推定において対応する周辺度数が一致するようにパラメータが決まることを意味する．
...
| 行の主効果 | $I-1$ | 行周辺が一致 |
| 列の主効果 | $J-1$ | 列周辺が一致 |
| 行×列の交互作用 | $(I-1)(J-1)$ | 全セルが一致（飽和） |

独立モデルや一様連関モデルは行・列の主効果を含むため，周辺分布は自動的に保持される．一方，一様連関の$\phi U_i V_j$は1自由度しか使わないため，セル度数は完全には一致しない．
:::
...
```{r}
# スコア変数を作成
d_occupationalStatus <- d_occupationalStatus |>
  mutate(Rscore = as.integer(origin),
         Rscore = Rscore - mean(Rscore),
         Cscore = as.integer(destination),
         Cscore = Cscore - mean(Cscore))

# 独立モデル
fit_O <- d_occupationalStatus |>
  gnm(Freq ~ origin + destination, family = poisson, data = _)

# 一様連関モデル
fit_U <- d_occupationalStatus |>
  gnm(Freq ~ origin + destination + Rscore:Cscore, family = poisson, data = _)
