...
- パッケージは`tidyverse`（データセットの処理のため），`magrittr`（パイプ演算子），`DescTools`（記述統計を求めるため），`vcd`パッケージ（カテゴリカルデータの分析のため），`gnm`（連関分析の処理のため）を使用する．
...
．`gnm`内で`data = .`となっているが`.`にはパイプ演算子の左辺にある`d`が入る．つまりこれは`data = d`とすることに等しい．
