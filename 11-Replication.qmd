...
d_yamaguchi <- d_yamaguchi |> 
  mutate(Diag = case_when(O == D ~ O, .default = "0"))
...
fit_y_4 <- d_yamaguchi |> 
  glm(Freq ~ O*C + D*C + Diag*C + U:V:C3, data = _, family = poisson)
glance(fit_y_4)
...
fit_y_11 <- d_yamaguchi |> gnm(Freq ~ O*C + D*C + Diag*C + MultHomog(O, D), 
...
fit_y_15 <- d_yamaguchi |> 
  glm(Freq ~ O*C + D*C + Diag*C + U:D + O:V + U:V*C, data = _, family = poisson)
glance(fit_y_15)
...
fit_y_18 <- d_yamaguchi |> 
  glm(Freq ~ O*C + D*C + Diag*C + SYM + U:V*C, data = _, family = poisson)
glance(fit_y_18)
...
fit_y_21 <- d_yamaguchi |> 
  glm(Freq ~ O*C + D*C + Diag*C + O:D + U:V*C, data = _, family = poisson)
glance(fit_y_21)
...
# R+C3: Uniform difference between G.B. andU.S.
# ...
```
