...
tibble(
  項目 = c("総サンプルサイズ", "セル数"),
  値 = c(sum(Freq), length(Freq))
) |>
  knitr::kable(caption = "データ概要")
```

## Model 1: 完全独立モデル

$$
\log \mu_{ijk} = \lambda + \lambda^E_i + \lambda^O_j + \lambda^I_k
$$
...
parameters {
  real lambda0;
  vector[3] lambda_educ;   // educ 2-4 (corner constraint)
  vector[11] lambda_occ;   // occ 2-12
  vector[3] lambda_income; // income 2-4
}
model {
  // Priors
  lambda0 ~ normal(0, 10);
  lambda_educ ~ normal(0, 5);
  lambda_occ ~ normal(0, 5);
  lambda_income ~ normal(0, 5);

  // Likelihood
  for (n in 1:N) {
    real log_mu = lambda0;
    if (educ[n] > 1) log_mu += lambda_educ[educ[n] - 1];
    if (occ[n] > 1) log_mu += lambda_occ[occ[n] - 1];
    if (income[n] > 1) log_mu += lambda_income[income[n] - 1];
...
tibble(
  推定法 = c("Stan", "gnm"),
  `L²` = c(fit1_mle$summary("G2")$estimate, deviance(m1_gnm))
) |>
  mutate(`L²` = round(`L²`, 2)) |>
  knitr::kable(caption = "Model 1: Stan MLE vs gnm")
```
...
tibble(
  項目 = c("観測度数", "gnm期待度数", "Stan期待度数"),
  合計 = c(sum(Freq), sum(fitted(m1_gnm)), sum(stan_expected))
) |>
  mutate(合計 = round(合計, 2)) |>
  knitr::kable(caption = "総度数の比較")

# 2. 切片の比較
tibble(
  推定法 = c("gnm", "Stan"),
  `λ₀` = c(gnm_coef["(Intercept)"], fit1_mle$summary("lambda0")$estimate)
) |>
  mutate(`λ₀` = round(`λ₀`, 6)) |>
  knitr::kable(caption = "切片の比較")
...
tibble(
  推定法 = c("gnm", "gnm", "gnm", "Stan", "Stan", "Stan"),
  計算方法 = rep(c("単純G²", "修正G²", "報告値"), 2),
  `G²` = c(G2_simple_gnm, G2_corrected_gnm, deviance(m1_gnm),
           G2_simple_stan, G2_corrected_stan, fit1_mle$summary("G2")$estimate)
) |>
  mutate(`G²` = round(`G²`, 2)) |>
  pivot_wider(names_from = 推定法, values_from = `G²`) |>
  knitr::kable(caption = "逸脱度の計算方法の比較")
```
...
parameters {
  real lambda0;
  vector[3] lambda_educ;
  vector[11] lambda_occ;
  vector[3] lambda_income;
}
model {
  // 事前分布なし（無情報事前分布）
  for (n in 1:N) {
...
tibble(
  推定法 = c("Stan (事前分布あり)", "Stan (事前分布なし)", "gnm"),
  `L²` = c(
    fit1_mle$summary("G2")$estimate,
    fit1_flat$summary("G2")$estimate,
    deviance(m1_gnm)
  ),
  `λ₀` = c(
    fit1_mle$summary("lambda0")$estimate,
    fit1_flat$summary("lambda0")$estimate,
    gnm_coef["(Intercept)"]
  ),
  `Σμ` = c(NA, fit1_flat$summary("total_expected")$estimate, sum(Freq))
) |>
  mutate(
    `L²` = round(`L²`, 2),
    `λ₀` = round(`λ₀`, 6),
  ...
  knitr::kable(caption = "事前分布の影響")
```

## Model 2: 条件付き独立モデル

$$
\log \mu_{ijk} = \lambda + \lambda^E_i + \lambda^O_j + \lambda^I_k + \lambda^{EO}_{ij} + \lambda^{OI}_{jk}
$$
...
parameters {
  real lambda0;
  vector[N_educ - 1] lambda_educ;
  vector[N_occ - 1] lambda_occ;
  vector[N_income - 1] lambda_income;
...
# L²比較: Stan = 上記で計算済み, gnm = deviance(m2_gnm)
```

## Model 3: すべての2元交互作用

$$
\log \mu_{ijk} = \lambda + \lambda^E_i + \lambda^O_j + \lambda^I_k + \lambda^{EO}_{ij} + \lambda^{OI}_{jk} + \lambda^{EI}_{ik}
$$
...
# L²比較: Stan = 上記で計算済み, gnm = deviance(m3_gnm)
```

## Model 4: RC(1)+RL(1) 部分連関

$$
\log \mu_{ijk} = \lambda + \lambda^E_i + \lambda^O_j + \lambda^I_k + \phi_{EO} \mu_{i1} \nu_{j1} + \phi_{OI} \nu_{j2} \eta_{k1}
$$
...
# L²比較: Stan = 上記で計算済み, gnm = deviance(m4_gnm)
```
...
$$
\log \mu_{ijk} = \lambda + \lambda^E_i + \lambda^O_j + \lambda^I_k + \phi \nu_j (\mu_i + \eta_k)
$$
...
# L²比較: Stan = 上記で計算済み, gnm = deviance(m5_gnm)
```
...
$$
\log \mu_{ijk} = \lambda + \lambda^E_i + \lambda^O_j + \lambda^I_k + \phi_{EO} \mu_i \nu_j + \phi_{OI} \nu_j \eta_k
$$
...
# Model 5'の結果（詳細は「結果のまとめ」セクションで比較）
```
...
# L²比較: Stan = 上記で計算済み, gnm = deviance(m6_gnm)
```
...
# L²比較: Stan = 上記で計算済み, gnm = deviance(m7_gnm)
```
...
# Model 7'の結果（詳細は「結果のまとめ」セクションで比較）
```
...
tibble(
  統計量 = c("L² (Stan)", "L² (本)", "φ_EO", "φ_OI", "φ_EI"),
  値 = c(
    fit8_mle$summary("G2")$estimate,
    177264.57,
    fit8_mle$summary("phi_EO")$estimate,
    fit8_mle$summary("phi_OI")$estimate,
    fit8_mle$summary("phi_EI")$estimate
  )
) |>
  mutate(値 = round(値, 4)) |>
  knitr::kable(caption = "Model 8（全スコア一貫）: Stan MLE")
```
...
tibble(
  Model = c("Model 5 (gnm式, φ共通)", "Model 5' (φ別々)"),
  `L²` = c(
    fit5_mle$summary("G2")$estimate,
    fit5p_mle$summary("G2")$estimate
  ),
  `φ` = c(fit5_mle$summary("phi")$estimate, NA),
  `φ_EO` = c(NA, fit5p_mle$summary("phi_EO")$estimate),
  `φ_OI` = c(NA, fit5p_mle$summary("phi_OI")$estimate)
) |>
  mutate(across(where(is.numeric), \(x) round(x, 4))) |>
  knitr::kable(caption = "Model 5 vs Model 5'")
```

L²の差は`r round(fit5_mle$summary("G2")$estimate - fit5p_mle$summary("G2")$estimate, 2)`（φ共通の制約による適合度の悪化）。
...
tibble(
  Model = c("Model 7 (gnm式, φ共通)", "Model 7' (φ別々)"),
  `L²` = c(
    fit7_mle$summary("G2")$estimate,
    fit7p_mle$summary("G2")$estimate
  ),
  `φ (E-O, O-I共通)` = c(fit7_mle$summary("phi")$estimate, NA),
  `φ_EO` = c(NA, fit7p_mle$summary("phi_EO")$estimate),
  `φ_OI` = c(NA, fit7p_mle$summary("phi_OI")$estimate),
  `φ_EI` = c(fit7_mle$summary("phi_EI")$estimate, fit7p_mle$summary("phi_EI")$estimate)
) |>
  mutate(across(where(is.numeric), \(x) round(x, 4))) |>
  knitr::kable(caption = "Model 7 vs Model 7'")
```

L²の差は`r round(fit7_mle$summary("G2")$estimate - fit7p_mle$summary("G2")$estimate, 2)`（φ共通の制約による適合度の悪化）。

### φの符号調整と解釈

```{r}
#| label: phi-adjustment

# スコアの符号調整（高い方がプラスになるよう）
...
tibble(
  連関 = c("φ_EO（教育-職業）", "φ_OI（職業-収入）"),
  `調整後φ` = c(phi_EO_adj, phi_OI_adj)
) |>
  mutate(`調整後φ` = round(`調整後φ`, 4)) |>
  knitr::kable(caption = "調整後のφ（高い方=プラス）")
```

φの比率: `r round(phi_EO_adj / phi_OI_adj, 2)` → 教育-職業連関は職業-収入連関の約`r round(abs(phi_EO_adj / phi_OI_adj), 1)`倍強い。
...
以下のようにλ₀を計算することで，総カウント制約を満たすことができる：
...
transformed parameters {
  real lambda0;
  {
    ...
    for (n in 1:N) {
      real lm = 0;
      // λ₀以外のすべてのパラメータを加算
      if (educ[n] > 1) lm += lambda_educ[educ[n] - 1];
      if (occ[n] > 1) lm += lambda_occ[occ[n] - 1];
      if (income[n] > 1) lm += lambda_income[income[n] - 1];
      // ... 他のパラメータも同様
      sum_exp += exp(lm);
    }
    // λ₀ = log(ΣY) - log(Σexp(他のパラメータ))
    lambda0 = log(total_Y) - log(sum_exp);
  }
}
```

この制約により，$\exp(\lambda_0) \cdot \sum_n \exp(\text{他のパラメータ}_n) = \sum_n Y_n$ が成立し，gnmと同等の結果が得られる．
...
この方法により，λ₀を自由パラメータとして推定しながら，gnmと同等のL²が得られる．
