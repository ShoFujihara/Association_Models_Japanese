...
# 図を並べる
ggpubr::ggarrange(fig_ct, fit_cs)
```
...
分析には通常は`glm`を用いるが，後の分析とあわせて`gnm`によって推定する．結果は異ならない．`family = poisson`という指定を忘れないようにすること．何も指定しないと`family = gaussian`となり，通常の線形回帰分析を行ってしまうので注意する．
