...
その前に集計データを`vcdExtra::expand.dft()`によって個票データに変換する．
...
df_residuals <- xtabs(data = df_tab_2.1) |> 
  chisq.test() |> 
  pluck("residuals") |> 
  data.frame() |> 
  rename(residuals = Freq) |> 
  mutate(sig = case_when(abs(residuals) >
                           qnorm(0.975) ~ 1,
                         .default = 0))

# ggplotでモザイクプロット
df_tab_2.1 |> 
  left_join(df_residuals) |> 
  ggplot() + 
  geom_mosaic(aes(x = product(Position, Member), fill = Position)) + 
